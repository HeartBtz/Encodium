...
After install, open **http://localhost:4000**, then use the credentials displayed by `install.sh` (random password generated by default) and add your media directories via **Settings → Sources**.
...
```bash
node cli.js scan      # Full scan + enrich (thumbnails are generated on-demand)
...
- **SSE pipeline events** — Dashboard and library auto-refresh when background scan/enrich completes; no page reload needed
